...
parser.add_argument('--location',type=str,help='GCP region',default='us-west1')
parser.add_argument('--prompt_id',type=str,help='Prompt dir name',default='prompts_a_5_02')
...
for prompt_system_f,prompt_user_f in pbar:
    ...
    prompt_audio = list()
    if 'train' in participant_id:
        for train_participant_id in participant_id.split('_')[3:]:
            f = prompts_user_d / f'{train_participant_id}.mp3'
            if not f.is_file():
                continue
            prompt_audio.append(Part.from_data(f.read_bytes(),mime_type='audio/mp3'))
    if 'val' in participant_id:
        val_participant_id = participant_id.split('_')[1]
    else:
        val_participant_id = participant_id
    f = prompts_user_d / f'{val_participant_id}.mp3'
    prompt_audio.append(Part.from_data(f.read_bytes(),mime_type='audio/mp3'))
    while True:
        try:
            ...
            response = model.generate_content(prompt_audio+[prompt_user])
